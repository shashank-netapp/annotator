...
type FuncInfo struct {
	...
	// RawSpec contains the raw Spec that declared this function
	RawSpec *ast.FuncDecl
	// RawFile contains the file in which this type was declared.
	RawFile *ast.File
}
...
type FunctionCallback func(funcInfo *FuncInfo)

// EachToken collects all markers, then calls the given callback for each type declaration in a package.
...
func EachToken(col *Collector, pkg *loader.Package, cb TypeCallback, fb FunctionCallback) error {
	markers, err := col.MarkersInPackage(pkg)
	if err != nil {
		return err
	}

	loader.EachType(pkg, func(file *ast.File, decl *ast.GenDecl, spec *ast.TypeSpec, funcDecl *ast.FuncDecl) {
		...
		if funcDecl != nil {
			fb(&FuncInfo{
				...
				Markers: markers[funcDecl],
				Doc:     extractDoc(funcDecl, decl),
				RawSpec: funcDecl,
				RawFile: file,
			})
		}
